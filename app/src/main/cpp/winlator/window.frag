...
void main() {
    ...
    c.rgb = clamp(c.rgb + vec3(pc.brightness), 0.0, 1.0);
    // Contrast: scale around 0.5 in [-1, 1] range
    c.rgb = clamp((c.rgb - 0.5) * (1.0 + pc.contrast) + 0.5, 0.0, 1.0);
    // Gamma correction
    if (pc.gamma > 0.0) c.rgb = pow(c.rgb, vec3(1.0 / pc.gamma));
