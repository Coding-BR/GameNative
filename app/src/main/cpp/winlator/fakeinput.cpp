...
namespace Logger {
	...
	void log(const char *message, ...) {
		...
		__android_log_vprint(ANDROID_LOG_INFO, "fakeinput", message, args);
		va_end(args);
	}
}
...
static void library_init() {
	...
	Logger::init();
	// Unconditional ctor log so we can verify the shim is loaded into a process.
	// Routes to logcat so it shows up alongside Java-side WinHandler logs.
	__android_log_print(ANDROID_LOG_INFO, "fakeinput",
		"library_init pid=%d hook_dir=%s vib=%d log=%d",
		getpid(), hook_dir, vibration_enabled, Logger::log_enabled);
}
